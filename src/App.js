...
function App() {
  ...
  const shuffleTeamsWithConstraints = () => {
    ...
    while (!validTeams) {
      ...
      const middleIndex = Math.ceil(shuffledMembers.length / 2);
  
      const team1 = shuffledMembers.slice(0, middleIndex);
      const team2 = shuffledMembers.slice(middleIndex);
      const isGulInTeam1 = team1.includes('귤');
      const isBreakerInTeam1 = team1.includes('브레이커');
  
      if (!(isGulInTeam1 && isBreakerInTeam1)) {
        setTeam1(team1);
        setTeam2(team2);
        validTeams = true;
      }
